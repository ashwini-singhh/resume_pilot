...
async def summarise_repo_for_resume(
    ...
    """
    from core.llm_client.llm_client import LLMClient  # local import to avoid circular
...
"""
    result = await llm_client.generate_json(prompt=prompt, temperature=0.2, max_tokens=600)
    if not result:
